...
- **Portfolio Agent prod ops (LIVE 2026-07-07):** the `/agent` route (`apps/web`) reads Railway Postgres pgvector directly as a least-privilege `agent_reader` role (SELECT on `CorpusChunk` only) over the Postgres **public TCP proxy** (`AGENT_DATABASE_URL=postgresql://agent_reader:<pw>@<RAILWAY_TCP_PROXY_DOMAIN>:<PORT>/railway?sslmode=no-verify`; Vercel is off Railway's private net). Ten runtime secrets live on Vercel prod+preview (OpenRouter/Google/Groq/Upstash x2/Langfuse x3/GitHub + `AGENT_DATABASE_URL`); the Google key ALSO serves in-container ingest (curation-only, pass inline to `ingest:corpus` or set as a Railway var, never rely on Vercel for ingest). The **API Dockerfile now ships `README.md` + `docs/`** so the in-container `ingest:corpus` reads the corpus sources (missing files are silently skipped, gutting the corpus). Content refresh after a seed/corpus change: push → Railway rebuild (pre-deploy `migrate deploy`) → `railway ssh -s portfolio "cd /app/apps/api && pnpm db:seed"` → `... GOOGLE_GENERATIVE_AI_API_KEY=<key> pnpm ingest:corpus"` → `... pnpm activity:refresh"` (seed writes a PLACEHOLDER snapshot; refresh restores the real one). Re-run `GRANT SELECT ON "CorpusChunk" TO agent_reader` after any migration that recreates the table. **$5 cap = the OpenRouter key credit limit + auto-top-up OFF** (monthly manual top-up; 402 → Groq `capped`, never an outage). Full runbook: `docs/deploy.md` → "Portfolio Agent — prod deploy runbook".
- **Docs map:** PRD `docs/PRD.md` · progress+decisions `docs/build-plan.md` · design spec `docs/m4-design-spec.md` · deploy `docs/deploy.md`. Per-feature PRDs → `docs/prd/`. Implementation backlogs (PRD → ordered items; template `docs/templates/implementation-backlog-template.md`) → `docs/backlogs/`; author with `/improvement-backlog`, execute item-by-item with `/backlog-implement`.
